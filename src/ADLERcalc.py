...
def simplify_number_range(flist):
    ...
    numbs = np.array(numbs).astype(np.int64)
...
def load_and_average_logs(flist):
    ...
    if 'TARGET' in vallog.keys():
        ...
        full = vallog[xkey]
        count = np.zeros(len(points)).astype(np.int64)
...
class RixsMeasurement():
    def __init__(self, filenames = [], max_threads = 1):
        ...
        self.the_header = defaultdict(lambda:[ -1])
        self.nfiles = 0
        self.times = []
        self.shortnames = []
        self.energy = -1.0
    ...
    @pyqtSlot(int)
    def setActive(self, cval):
        if cval == Qt.CheckState.Unchecked:
            self.active = False
        else:
            self.active = True
    ...
    def write_extended_ADLER(self, fname):
        ...
        if header is not None:
            for hk in header.keys():
                tkey = str(hk)
                newln = tkey + " $ " + "".join([str(x) for x in header[hk] if str(x).isprintable()])
    ...
    def read_extended_ADLER(self, fname):
        data = {'header': defaultdict(lambda: [-1]),
                    'log':defaultdict(lambda: np.array([])),
                    'crucial': defaultdict(lambda: ['-1']), 
                    'history' : []}
        profile = []
        lognames = []
        logvals = []
        ...
        for line in target:
            ...
            if len(toks) > 1:
                ...
                if group in ['header']:
                    ...
                    key,  value = entry.split('$')[0].strip(' '),  entry.split('$')[1].strip(' ')
                    if not key in data[group].keys():
                        data[group][key] = [value]
                    else:
                        data[group][key].append(value)
                elif group in ['log']:
                    values = entry.split()
                    if len(values) < 5:
                        continue
        ...
        self.the_header = data['header']
        self.the_log = defaultdict(lambda: np.array([]))
        for n,  key in enumerate(lognames):
            self.the_log[key] = logvals[:, n]
        self.history = data['history']
        self.assignCrucialParts(data['crucial'])
    ...
    def subtractDarkCurrent(self, scaling = 1.0):
        if self.tdb_profile is None:
                try:
                    source = open('BKG_OVERRIDE.txt', 'r')
                except:
                    source = open(resource_path('Structured_bkg_per_second.dat'), 'r')
                arr = []
                for line in source:
