...
from PyQt5.QtCore import pyqtSlot, pyqtSignal, QSize,  QThread
from PyQt5.QtGui import QFont
from PyQt5.QtWidgets import QFrame, QSizePolicy, QWidget, QFileDialog,   \
...
def plot1D(pic, outFile = "", fig = None, text = '', label_override = ["", ""], curve_labels= [],  title = "", autolimits = True):
    if fig == None:
        fig = mpl.figure(figsize = [12.0, 8.0], dpi=75, frameon = False)
        trigger = True
    else:
...
{'Name': 'Detector cutoff',  'Unit':'pixel',  'Value':np.array([0, 2048]),  'Key' : 'cuts', 
                               'MinValue':np.array([0, 0]),  'MaxValue':np.array([4096, 4096]),  'Length': 2,  'Type':'int', 
...
class SingleTab(AdlerTab):
    ...
    def make_layout(self):
        ...
        # self.base=base
        base = self.base
        base.setSizePolicy(QSizePolicy.Minimum, QSizePolicy.Expanding)
        ...
        button_layout.setFieldGrowthPolicy(QFormLayout.AllNonFixedFieldsGrow)
        ...
        self.active_buttons = np.zeros(len(button_list))
...
#            self.button_list[n].setEnabled(self.active_buttons[n])
    ...
    def MakeCanvas(self, parent):
        ...
        figAgg.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Expanding)
...
#            self.logplotter.takeData(self.core.logvals)
    ...
    def fake_data_button(self):
        ...
        self.active_buttons[0:9] = 1
        self.active_buttons[9:12] = 0
        self.active_buttons[12:14] = 1
    @pyqtSlot()
    def show_1D_comparison(self):
        profs = [self.core.summed_rawprofile,  self.core.summed_adjusted_rawprofile]
        plot1D(profs, fig = self.figure, text = "Pick the better profile!", title = self.core.temp_name, 
    ...
    def after_fft(self):
        if self.core.fft_applied:
            ...
            self.active_buttons[13:14] = 1
            self.flip_buttons()
    def correct_curvature(self):
        self.block_interface()
        self.active_buttons[12:14] = 0
        self.for_poly.emit()
