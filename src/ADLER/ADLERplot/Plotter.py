...
class Plotter:
    ...
    def plot1D(
        ...
        axtextf = self.figure.add_axes(
            [0.40, 0.01, 0.20, 0.01], frameon=False
        )  # , axisbg = '0.9')
        axtextf.set_yticks([])
        axtextf.set_xticks([])
    ...
    def plot1D_sliders(
        ...
        if len(curve_labels) == len(pic):
            axes.legend(loc=legend_pos)
        # here we add sliders
        offset_slider_ax = self.figure.add_axes(
            [0.25, 0.15, 0.55, 0.03]
        )  # , axisbg=axis_color)
    ...
    def plot1D_grid(
        self, pic, outFile="", fig=None, text="", label_override=["", ""], legend_pos=0
    ):
        if self.figure == None:
            self.figure = mpl.figure(figsize=[12.0, 8.0], dpi=75, frameon=False)
            trigger = True
    ...
    def plot2D_sliders(
        self, pics, ax, outFile="", fig=None, text="", interp="none", labels=["", ""]
    ):  # interp = 'Bessel'):
        if self.figure == None:
            self.figure = mpl.figure(figsize=[12.0, 8.0], dpi=75, frameon=False)
            trigger = True
        ...
        xxx = axes.imshow(
            np.nan_to_num(pic)[::-1, :],
            extent=[ax[1][0], ax[1][-1], ax[0][0], ax[0][-1]],
            interpolation=interp,
            cmap=self.colourmap,
            aspect="auto",
            vmin=np.percentile(pic, 20),
            vmax=np.percentile(pic, 90),
            # vmin = 1e-3, vmax = 1.0
        )
        ...
        ty1, ty2 = axes.get_ylim()
        tpos_y = ty2 + 0.05 * (ty2 - ty1)
        maxval_slider_ax = self.figure.add_axes(
            [0.12, 0.12, 0.55, 0.03]
        )  # , axisbg=axis_color)
        maxval_slider = Slider(maxval_slider_ax, "Maxval", 0.0, 100.0, valinit=90.0)
        minval_slider_ax = self.figure.add_axes(
            [0.12, 0.04, 0.55, 0.03]
        )  # , axisbg=axis_color)
    ...
    def plot1D_merged(
        ...
        for rn in range(len(pic)):
            ...
            [ref2, dummy1, dummy2] = axes.errorbar(
                p[:, 0], p[:, 1], yerr=p[:, 2], color=tempcolour
            )
