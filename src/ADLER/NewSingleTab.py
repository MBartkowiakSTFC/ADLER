...
loading_variables = [
    {
        "Name": "Detector cutoff LRBT",
        "Unit": "pixel",
        "Value": np.array([0, 2048, 0, 2048]),
        "Key": "cuts",
        "MinValue": np.array([0, 0, 0, 0]),
        "MaxValue": np.array([8192, 8192, 8192, 8192]),
        "Length": 4,
        "Type": "int",
    ...
    {
        "Name": "Time-dependent background",
        "Unit": "fraction",
        "Value": 1.0,
        "Key": "tdb",
        "MinValue": 0.0,
...
class SingleTab(AdlerTab):
    ...
    def __init__(
        ...
        self.core = AdlerCore(
            ...
            thr_exit=self.unblock_interface,
            startpath=startpath,
    ...
    def make_layout(self):
        ...
        #
        self.calib_model = PeaxisDataModel(
            boxes_base,
            ["Filename", "Line position [channels]", "Beam energy [eV]", "Date"],
        )
        ...
        for n in range(5):
            line = [
                QStandardItem(str(x)) for x in ["---", "-1.0", "-1.0", "DD/MM/YYYY"]
            ]
        ...
        button_list = [
            [
                ...
                "Pick the binary file to be processed.",
                col1,
                "Files",
            ],  # 0
            [
            ...
            [
                "Get Calibration",
                self.load_calibration_from_files,
                "Read energy calibration from existing output files.",
                "",
                "Files",
            ],  # 2
            [
            ...
            [
                "Fast Fit",
                self.core.autoprocess_file,
                "Create a 1D profile, guess line position.",
                col1,
                "Processing",
            ],  # 3
            [
                "Make Profile",
                self.core.process_file,
                "Create a 1D profile.",
                col1,
                "Processing",
            ],  # 4
            [
                "Energy Fast Fit",
                self.core.auto_eV_profile,
                "Same as Fast Fit, but using energy units on the x axis.",
                col2,
                "Processing",
            ],  # 5
            [
                "Energy Profile",
                self.core.eV_profile,
                "Same as Make Profile, but using energy units on the x axis.",
                col2,
                "Processing",
            ],  # 6
            [
                "Plot Curvature",
                self.core.curve_profile,
                "Plot position of the peak as a function of horizontal position.",
                col3,
                "QC",
            ],  # 7
            [
                "Plot Horizontal",
                self.core.make_stripe,
                "Plot the intensity profile of the elastic line across the detector.",
                col3,
                "QC",
            ],  # 8
            [
                ...
                "Stack up all the files, pixel per pixel.",
                col1,
                "Merging",
            ],  # 9
            [
                "Shifted Merge",
                self.merge_files_offsets,
                "Shift the files relative to each other to find the best match.",
                col1,
                "Merging",
            ],  # 10
            [
                "Expensive Merge",
                self.merge_files_expensive,
                "Shift the files relative to each other using a global minumum search algorithm.",
                col1,
                "Merging",
            ],  # 11
            [
            ...
            [
                "Fake Data",
                self.fake_data_button,
                "Look for processing artifacts by working on random numbers.",
                col3,
                "QC",
            ],  # 14
        ]
    ...
    def load_file_button(self):
        result, ftype = QFileDialog.getOpenFileNames(
            self.master,
            "Load data from one or more Andor camera files:",
            self.currentpath,
    ...
    def show_1D_comparison(self):
        self.plotter.plot_new_profiles(
            separate=self.core.individual_profiles,
            ...
            separate_labels=self.core.individual_labels,
            merged_labels=["Simple Merge", "Shifted Merge"],
    ...
    def show_2D_plot(self):
        if self.core.timedata[0] > 1:
            ...
            descr = " file"
        titlebar = (
            "+".join([str(x) for x in np.unique(self.core.energies)])
            + " eV, "
            + str(self.core.timedata[1].sum())
            + " s in "
            + str(self.core.timedata[0])
            + descr
            + "\n"
            + self.core.temp_name
        )
        self.plotter.plot2D_sliders(
            self.core.data2D, self.core.plotax, fig=self.figure, text=titlebar
        )
    ...
    def plot_fitting_results(self, fittinglist):
        ...
        if profi is None:
            ...
            self.plotter.plot1D(
                ...
                text="",
                title=titlebar,
                label_override=[self.core.chan_override, "Counts"],
                curve_labels=["Data"],
                eline=self.core.nom_eline,
    ...
    def load_calibration_from_files(self):
        ...
        results, ftype = QFileDialog.getOpenFileNames(
            self.master,
        ...
        if len(results) < 1:
            ...
            for result in results:
                ...
                try:
                    ...
                    date = "--/--/----"
                tableline = [
                    QStandardItem(str(x)) for x in [name, position, energy, date]
                ]
            ...
            try:
                ...
                for line in source:
                    ...
                    if "Calibration:" in line:
                        calibration.append(
                            "Calibration:".join(line.split("Calibration:")[1:])
                        )
    ...
    def plot_curve_profile(self, curvelist):
        if self.core.curvature is not None:
            ...
            self.plotter.plot1D(
                [curve, fit],
                fig=self.figure,
                text=self.core.temp_name,
                title=titlebar,
                label_override=["Channels", "Channels"],
                curve_labels=["Elastic line position", "Curvature fit"],
            )
    ...
    def segment_plot_button(self):
        ...
        if self.core.eline[0] < 0 or self.core.eline[1] < self.core.eline[0]:
            self.logger(
                "The elastic line limits are ill-defined. Not processing further."
            )
            return None
    ...
    def plot_segment(self, seglist):
        ...
        self.plotter.plot1D(
            [stripe],
            fig=self.figure,
            title=self.core.temp_name,
            text="Horizontal profile, rows "
            + str(self.core.eline[0])
            + " to "
            + str(self.core.eline[1]),
            label_override=["Channels", "Counts"],
            curve_labels=["stripe 1"],
        )
    ...
    def plot_energy_results(self, fittinglist):
        ...
        if profi is None:
            ...
            self.plotter.plot1D(
                ...
                text="",
                title=titlebar,
                label_override=["Energy [eV]", "Counts"],
                curve_labels=["Data"],
                efactor=(round(self.core.the_object.mev_per_channel, 3)),
            )
        else:
            ...
            self.plotter.plot1D(
                ...
                text=text,
                title=titlebar,
                label_override=["Energy transfer [eV]", "Counts"],
                curve_labels=["Data", "Background", "Fit"],
                efactor=(round(self.core.the_object.mev_per_channel, 3)),
            )
