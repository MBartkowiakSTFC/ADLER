...
from ADLER.ADLERcalc.FitCore import FitCore
...
loading_variables = [
    {
        "Name": "Curve cutoff",
        "Unit": "?",
        "Value": np.array([-1e5, 1e5]),
        "Key": "cuts",
        "MinValue": -1e15 * np.array([1.0, 1.0]),
        "MaxValue": 1e15 * np.array([1.0, 1.0]),
        "Length": 2,
        "Type": "float",
...
fit_variables = [
    ...
    {
        "Name": "Background polynomial order",
        "Unit": "",
        "Value": 1,
        "Key": "polyorder",
        "MinValue": 0,
...
class FittingTab(AdlerTab):
    ...
    def make_layout(self):
        ...
        button_list = [
            ...
            [
                "Sequential Fit",
                self.sequential_fit,
                "Fit the curve using a iterative algorithm.",
                col1,
                "Fitting",
            ],  # 2
    ...
    def finish_loading(self, parlist):
        ...
        self.plotter.plot1D(
            [profile],
            fig=self.figure,
            text="Choose the fitting parameters.",
            label_override=["X axis", "Y axis"],
            curve_labels=[shortname],
        )
        self.flip_buttons()
