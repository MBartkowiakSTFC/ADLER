...
from ADLER.ADLERcalc.xasUtils import (
    fit_neon_gas,
    fit_n2_gas,
    fit_edge_profile,
    gauss_denum,
...
fitting_variables = [
    {
        "Name": "Fixed Lorentz FWHM for N2",
        "Unit": "meV",
        "Value": 120,
        "Key": "n2width",
        "MinValue": 0.0,
    ...
    {
        "Name": "Fixed Lorentz FWHM for Neon",
        "Unit": "meV",
        "Value": 254,
        "Key": "neonwidth",
        "MinValue": 0,
    ...
    {
        "Name": "Number of linear functions for Edge",
        "Unit": "N/A",
        "Value": 0,
        "Key": "nlines",
        "MinValue": 0,
...
class BeamlineTab(AdlerTab):
    ...
    def make_layout(self):
        ...
        button_list = [
            ...
            [
                "Fit Neon",
                self.fit_neon,
                "Fit the position of the Ne peak for beamline energy calibration.",
                col3,
                "Fitting",
            ],  # 0
            [
                "Fit Nitrogen",
                self.fit_nitrogen,
                "Fit the width of the N2 peaks for the beamline resolution test.",
                col3,
                "Fitting",
            ],  # 1
            [
                "Fit Edge",
                self.fit_edge,
                "Fit the curve with a step function.",
                col3,
                "Fitting",
            ],  # 1
        ]
    ...
    def fit_neon(self):
        ...
        ypoints = y[1]
        try:
            profile, gauss_sigma, lorentz_gamma, peak_pos, GOF = fit_neon_gas(
                np.column_stack([xpoints, ypoints]),
                fixed_gamma=self.fwhm_neon / 1000.0 / 2.0,
            )
        except:
            profile, gauss_sigma, lorentz_gamma, peak_pos, GOF = (
                np.zeros(xpoints.shape),
                -1.0,
                -1.0,
                [],
                1e9,
        ...
        res_text = (
            "Gauss FWHM = "
            + str(round(gauss_sigma * 1000.0 * gauss_denum, 4))
            + " meV\n"
            + "Lorentz FWHM = "
            + str(round(lorentz_gamma, 4) * 2000.0)
            + " meV\n"
            + "Peak positions = "
            + ", ".join([str(x) for x in np.round(peak_pos, 3)])
            + "\n"
            + "Fit Quality = "
            + str(round(1.0 / GOF, 3))
        )
        ...
        self.plotter.plot1D(
            [np.column_stack([xpoints, ypoints]), np.column_stack([xpoints, profile])],
            outFile="",
            fig=self.figure,
            text=res_text,
            label_override=[xlab, ylab],
            curve_labels=["Data", "Fit"],
            legend_pos=0,
        )
    ...
    def fit_nitrogen(self):
        ...
        ypoints = y[1]
        try:
            profile, gauss_sigma, lorentz_gamma, peak_pos, GOF = fit_n2_gas(
                np.column_stack([xpoints, ypoints]),
    ...
    def fit_edge(self):
        ...
        ypoints = y[1]
        try:
            profile, edge_pos, edge_width, GOF = fit_edge_profile(
                np.column_stack([xpoints, ypoints]), nlines=self.nlines_edge
            )
        except:
            profile, edge_pos, edge_width, GOF = (
                np.zeros(xpoints.shape),
                -1.0,
                -1.0,
                1e9,
        ...
        self.plotter.plot1D(
            ...
            fig=self.figure,
            text="Edge position = "
            + str(round(edge_pos, 4))
            + "\n"
            + "Edge width = "
            + str(round(edge_width, 4))
            + "\n"
            + "Fit Quality = "
            + str(round(1.0 / GOF, 3)),
            label_override=[xlab, ylab],
    ...
    def load_LISE(self):
        result, ftype = QFileDialog.getOpenFileNames(
            self.master,
            "Load LISE scan files (.txt files):",
            self.currentpath,
