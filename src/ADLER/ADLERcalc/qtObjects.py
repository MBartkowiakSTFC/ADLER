...
def quick_match_profiles(
    data, to_match, xshift=0.0, tpoolin=None, pbarin=None, maxthreads=1
):
    tempdat = data.copy()
    tempdat[:, 0] -= xshift
    ...
    the_object = MergeCurves(
        tempdat, temptarget, None, pbar=pbarin, mthreads=maxthreads
    )
    the_object.runit()
    rebinned = the_object.postprocess()
    pfit = leastsq(
        scaling_fit, [1.0, 0.0], args=(rebinned[10:-10, 1], to_match[10:-10, 1])
    )
...
class ShiftOneProfile(QObject):
    ...
    def run(self):
        results = shgo(
            shgo_profile_offsets,
            [(-self.spread, self.spread)],
            args=(self.p, self.match, None, None, 1),
...
class LoadAndorFile(QObject):
    ...
    def run(self):
        ...
        tadded_header, units_for_header = load_datheader(
            os.path.join(self.fpath, self.nameroot + ".dat")
        )
    ...
    def __init__(self, source, target, tpool=None, pbar=None, mthreads=1):
        ...
        for nt in range(nthreads):
            self.runnables.append(
                CurveMergeWorker(
                    limits[nt], limits[nt + 1], self.arrays, self.consts, self
                )
            )
...
class MergeManyArrays(QObject):
    def __init__(self, arrays, offsets, tpool=None, pbar=None, mthreads=1):
        ...
        for nt in range(len(self.packages)):
            self.runnables.append(
                ArrayMergeWorker(self.packages[nt], self.arrays, self)
            )
...
class MergeManyCurves(QObject):
    def __init__(self, sources, target, tpool=None, pbar=None, mthreads=1):
        ...
        for nt in range(len(sources)):
            self.runnables.append(
                CurveAltMergeWorker(self.packages[nt], self.arrays, self.consts, self)
            )
...
class ArrayMergeWorker(QObject):
    ...
    def fastrun(self):
        ...
        overlap = 1.0 - np.abs(
            (limits2.reshape((len(limits2), 1)) - limits1.reshape((1, len(limits1))))
            / step1.reshape((1, len(step1)))
        )
...
class CurveMergeWorker_plain(QObject):
    ...
    def fastrun(self):
        ...
        prescreen = highlim1.reshape((1, len(highlim1))) >= lowlim2.reshape(
            (len(lowlim2), 1)
        )
        prescreen = np.logical_and(
            prescreen,
            lowlim1.reshape((1, len(lowlim1))) <= highlim2.reshape((len(highlim2), 1)),
        )
        # part1 = (highlim2.reshape((len(highlim2), 1)) - lowlim2.reshape((len(lowlim2), 1)) )
        part2 = (
            highlim1.reshape((1, len(highlim1))) - lowlim2.reshape((len(lowlim2), 1))
        ) / step1[:-1].reshape((1, len(lowlim1)))
        part3 = highlim2.reshape((len(highlim2), 1)) - lowlim1.reshape(
            (1, len(lowlim1))
        )
        ...
        crit2 = part2 <= 1.0
        crit3 = part3 <= 1.0
        overlap[np.where(np.logical_and(crit2, prescreen))] = part2[
            np.where(np.logical_and(crit2, prescreen))
        ]
        overlap[np.where(np.logical_and(crit3, prescreen))] = part3[
            np.where(np.logical_and(crit3, prescreen))
        ]
...
class LoadFileList(QObject):
    ...
    def postprocess(self):
        ...
        for hk in header.keys():
            ...
            textheader += " ".join(
                ["#", hk]
                + [str(round(x, 4)) for x in [tmin, tmax, tmean, tstd]]
                + ["\n"]
            )
