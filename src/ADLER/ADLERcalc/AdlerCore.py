...
from ADLER.ADLERcalc.ioUtils import (
    WriteProfile,
    simplify_number_range,
    resource_path,
    simple_read,
)
from ADLER.ADLERcalc.fitUtils import polynomial, fit_polynomial, gauss_denum
from ADLER.ADLERcalc.imageUtils import (
    elastic_line,
    curvature_profile,
    make_profile,
    apply_offset_to_2D_data,
    SphericalCorrection,
    make_histogram,
    make_stripe,
)
...
from ADLER.ADLERcalc.qtObjects import (
    MergeCurves,
    ShiftProfilesParallel,
    Worker,
    CustomThreadpool,
)
...
class NewAdlerCore(QObject):
    ...
    def __init__(
        ...
        self.the_object = RixsMeasurement(
            max_threads=max_threads, tdb_profile=self.tdb_profile
        )
    ...
    def read_tdb_profile(self, fname="BKG_OVERRIDE.txt"):
        try:
            ...
            self.logger(
                "Did not find an override file BKG_OVERRIDE.txt - using built-in background from June 2020."
            )
            source = open(DATA._files["Structured_bkg_per_second"], "r")
        else:
            self.logger(
                "Loading background per channel per second from BKG_OVERRIDE.txt"
            )
        arr = []
        for line in source:
    ...
    def load_files(self, flist):
        if len(flist) > 0:
            ...
            self.the_object = RixsMeasurement(
                max_threads=self.maxthreads, tdb_profile=self.tdb_profile
            )
    ...
    def adjust_offsets_automatically(self, tolerance=10.0):
        ...
        for s in shifted:
            ...
            the_object = MergeCurves(
                s, temp2, tpool=None, pbar=self.progbar, mthreads=self.maxthreads
            )
            the_object.runit()
    ...
    def preprocess_manyfiles(self, flist):
        ...
        for n in range(len(fnames)):
            self.individual_labels.append(
                str(round(energies[n], 3)) + " eV, " + fnames[n]
            )
    ...
    def curve_profile(self):
        ...
        if self.data2D is not None:
            curve = curvature_profile(
                self.data2D,
                blocksize=self.segsize,
                percentile=self.bkg_perc,
                olimits=self.eline,
            )
            pfit, pcov, infodict, errmsg, success = leastsq(
                fit_polynomial,
    ...
    def calculate_fft(self):
        ...
        fft_min, fft_max, fft_mean, fft_sum = (
            tempdata.min(0),
            tempdata.max(0),
            tempdata.mean(0),
            np.abs(tempdata).sum(0),
        )
    ...
    def correct_fft(self):
        ...
        filler = interp1d(
            np.concatenate([x_freqs[m1 - 3 : m1], x_freqs[m2 : m2 + 3]]),
    ...
    def apply_poly(self):
        ...
        if data is None:
            self.logger(
                "No data available to which curvature correction could be applied."
            )
        ...
        if poly is not None:
            if poly[0] < -99.0 and poly[1] < -99.0 and poly[2] < -99.0:
                self.logger(
                    "Please specify some realistic parameters for the curvature correction."
                )
                self.finished_poly.emit()
                self.did_nothing.emit()
                return None
                ...
                curve[:, 0] -= 0.5
                newcurve = interp1d(curve[:, 0], curve[:, 1], fill_value="extrapolate")(
                    np.arange(lcut, hcut)
                )
                data = SphericalCorrection(
                    data, poly, locut=lcut, hicut=hcut, direct_offsets=newcurve
                )
            else:
                data = SphericalCorrection(data, poly, locut=lcut, hicut=hcut)
            self.data2D = data
            self.processing_history.append(
                ["CurvatureCorrectionApplied", poly[0], poly[1], poly[2]]
            )
    ...
    def make_stripe(self):
        ...
        self.segmentresult.emit(
            [
                self.segment_plot,
                [(self.eline[0], self.eline[1]), (self.cuts[0] + 1, self.cuts[1])],
            ]
        )
        return self.segment_plot, [
            (self.eline[0], self.eline[1]),
            (self.cuts[0] + 1, self.cuts[1]),
        ]
    ...
    def make_histogram(self):
        ...
        WriteProfile(
            os.path.join(self.temp_path, self.temp_name + "_HISTOGRAM.txt"), temphist
        )
    ...
    def process_file(self, guess=False, final=True):
        ...
        self.plotax1D = [
            (self.cuts[2], self.cuts[3]),
            (self.cuts[0] + 1, self.cuts[1] + 1),
        ]
        self.the_object.write_extended_ADLER(
            os.path.join(self.temp_path, self.temp_name + "_extended1D.txt")
        )
        ...
        self.fitting_params_channels = [
            [tempdict["maxval"], tempdict["fwhm"], tempdict["centre"]],
            [
                tempdict["maxval_error"],
                tempdict["fwhm_error"],
                tempdict["centre_error"],
            ],
        ]
        ...
        if final:
            ...
            if fit is not None:
                self.params_fitting.emit(
                    {
                        "FIT_centre": np.array(
                            [round(fit[0][2], 3), abs(round(fit[1][2], 3))]
                        ),
                        "FIT_fwhm": np.array(
                            [abs(round(fit[0][1], 3)), abs(round(fit[1][2], 3))]
                        ),
                        "FIT_area": np.array(
                            [round(peak_area, 3), abs(round(peak_area_error, 3))]
                        ),
                    }
                )
    ...
    def eV_profile(self, guess=False):
        ...
        if eguess is None:
            ...
            fit = [
                [temp["maxval"], temp["fwhm"], temp["centre"]],
            ...
            self.energyresult.emit(
                [self.energy_profile.copy(), back, peak, fit, fitstring]
            )
            self.params_energy.emit(
                {
                    "ENERGY_centre": np.array([0.0, abs(round(1000.0 * fit[1][2], 3))]),
                    "ENERGY_fwhm": np.array(
                        [
                            abs(round(1000.0 * fit[0][1], 3)),
                            abs(round(1000.0 * fit[1][1], 3)),
                        ]
                    ),
                    "ENERGY_area": np.array(
                        [round(peak_area, 3), abs(round(peak_area_error, 3))]
                    ),
                }
            )
