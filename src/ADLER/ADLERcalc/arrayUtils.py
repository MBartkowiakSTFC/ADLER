...
def discrete_rebin(data: np.array, offset: float = 0.0, nsubdiv: int = 100) -> np.array:
...
def place_data_in_bins(bigarray, new_limits):
    ...
    for nn in range(len(points)):
        count[nn] += len(
            bigarray[
                np.where(
                    np.logical_and(
                        bigarray[:, 0] >= new_limits[nn],
                        bigarray[:, 0] < new_limits[nn + 1],
                    )
                )
            ]
        )
    ...
    for counter in range(len(points)):
        subset = bigarray[:, 1][
            np.where(
                np.logical_and(
                    bigarray[:, 0] < new_limits[counter + 1],
                    bigarray[:, 0] >= new_limits[counter],
                )
            )
        ]
        if count[counter] > 0:
        ...
        for nn in range(len(points)):
            count[nn] += len(
                full[
                    np.where(
                        np.logical_and(
                            full >= new_limits[nn], full < new_limits[nn + 1]
                        )
                    )
                ]
            )
        new_limits = np.concatenate(
            [new_limits[:1], new_limits[1:][np.where(count > 0)]]
        )
        for k in vallog.keys():
            temp = vallog[k]
            newone, newerr = [], []
            for counter in range(len(points)):
                subset = temp[
                    np.where(
                        np.logical_and(
                            full < new_limits[counter + 1], full >= new_limits[counter]
                        )
                    )
                ]
