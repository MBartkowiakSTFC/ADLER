...
class XasCorrector(QObject):
    def __init__(self, parent=None):
        ...
        self.limits_harm1 = [
            0,
            0,
        ]  # the total limits of data available in our referece file for this harmonics
    ...
    def returnInterpolatedCurve(
        self, xarray, kind_covered="linear", kind_notcovered="cubic"
    ):
        """
        ...
        for nh in range(4):
            ...
            match_deg = np.logical_and(
                inarr >= abslims[0], inarr <= abslims[1]
            ).sum() / len(inarr)
            match[nh] = (
                match_deg  # we check the overlap between the x range of the input data and reference data
            )
        match = np.array(match)
        if np.all(match == 0.0):
            print(
                "None of the data ranges match the input data range. Flux correction is not possible."
            )
            return None
