...
def single_neon_profile(
    params, xarr, fixed_lorentz=0.254, yarr=None, extraweight=False
):
    ...
    profile += (
        abs(params[-4]) * np.arctan((xarr - params[-3]) / params[-2]) + params[-1]
    )
    if yarr is not None:
        profile -= yarr
        if extraweight:
...
def fit_edge_profile(num_arr, nlines=0):
    ...
    pfit, pcov, infodict, errmsg, success = leastsq(
        single_edge_profile,
        params,
        args=(num_arr[:, 0], 0.254, num_arr[:, 1]),
        full_output=1,
    )
    final_profile = single_edge_profile(pfit, num_arr[:, 0], 0.254)
    GOF = ((final_profile - num_arr[:, 1]) ** 2).sum() ** 0.5 / np.abs(
        num_arr[:, 1]
    ).sum()
    print(
        "GOF: ",
        GOF,
        ((final_profile - num_arr[:, 1]) ** 2).sum() ** 0.5,
        np.abs(num_arr[:, 1]).sum(),
    )
...
def fit_n2_gas(inp_num_arr, fixed_gamma=0.06):
    peak_list = np.array(
        [400.76, 400.996, 401.225, 401.45, 401.66, 401.88, 402.1, 402.29, 402.48]
    )
    params = []
    # fixed_gamma = 0.012
    ...
    pfit, pcov, infodict, errmsg, success = leastsq(
        single_n2_profile,
        params,
        args=(num_arr[:, 0], fixed_gamma, num_arr[:, 1], True),
        full_output=1,
    )
