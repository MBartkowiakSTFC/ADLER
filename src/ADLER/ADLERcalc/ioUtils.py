        ...
        for kk in vallog.keys():
            ...
            if keystring in [
                ...
                if abs(stepvar) < 1e-6 and abs(stepsize) < 1e-6:
                    continue
                costfunc = np.nan_to_num(
                    np.array([stepvar]) / np.array([abs(stepsize)])
                )[
                    0
                ]  # correct it!
...
def header_read(fname):
    ...
    tadded_header, units_for_header = load_datheader(
        os.path.join(fpath, nameroot + ".dat")
    )
    tadded_log = load_datlog(os.path.join(fpath, nameroot + ".xas"))
...
def load_and_average_logs(flist):
    ...
    if "TARGET" in vallog.keys():
        ...
        for nn in range(len(points)):
            count[nn] += len(
                full[
                    np.where(np.logical_and(full >= limits[nn], full < limits[nn + 1]))
                ]
            )
        ...
        for k in vallog.keys():
            ...
            for counter in range(len(newpoints)):
                subset = temp[
                    np.where(
                        np.logical_and(
                            full < limits[counter + 1], full >= limits[counter]
                        )
                    )
                ]
...
def WriteProfile(fname, profile, header=None, params=[None, None], varlog=[None, None]):
    target = open(fname, "w")
        ...
        for kk in keys:
            ...
            target.write(
                " ".join(
                    [
                        str(x)
                        for x in ["# ADLER", kk, ":", val.strip().strip("()[]\n"), "\n"]
                    ]
                )
            )
