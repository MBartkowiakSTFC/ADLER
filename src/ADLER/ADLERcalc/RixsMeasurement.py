...
class RixsMeasurement:
    ...
    def summariseCrucialParts(self):
        ...
        pdict["fitting_params_absEnergy"] = [
            " ".join([str(k), str(self.fitting_params_absEnergy[k])])
            for k in self.fitting_params_absEnergy
        ]
        pdict["fitting_params_channels"] = [
            " ".join([str(k), str(self.fitting_params_channels[k])])
            for k in self.fitting_params_channels
        ]
        pdict["fitting_params_eV"] = [
            " ".join([str(k), str(self.fitting_params_eV[k])])
            for k in self.fitting_params_eV
        ]
    ...
    def assignCrucialParts(self, pdict):
        ...
        self.calibration_params = [
            float(x) for x in pdict["calibration_params"][0].split()
        ]
    ...
    def write_ADLER(self, fname):
        ...
        for nn, fitdict in enumerate(
            [
                self.fitting_params_channels,
                self.fitting_params_absEnergy,
                self.fitting_params_eV,
            ]
        ):
            suffix = suff[nn]
            ...
            newheader += [
                ...
                + ",".join(
                    [
                        ...
                        for x in [
                            self.calibration[n],
                            self.calibration_source[n],
                            self.calibration_dates[n],
                        ]
                    ]
                )
            ]
        ...
        if len(self.profile_channels) > 0:
            WriteProfile(
                ...
                header=newheader,
                params=[parlist, pardict],
                varlog=[logvalnames, self.the_log],
            )
        if len(self.profile_absEnergy) > 0:
            WriteProfile(
                fname + "_1D_absEnergy.txt",
                self.profile_absEnergy,
                header=newheader,
                params=[parlist, pardict],
                varlog=[logvalnames, self.the_log],
            )
                ...
                if "calibration" in tkey:
                    for entry in crucial[hk]:
                        try:
                            ...
                            newln = (
                                ...
                                + " $ "
                                + " ".join(
                                    [str(x) for x in entry if str(x).isprintable()]
                                )
                            )
                        else:
                            newln = (
                        ...
                        target.write(
                            " ".join(["#CRUCIAL#", newln.strip().strip("\n"), "\n"])
                        )
                    ...
                    try:
                        ...
                        newln = (
                            ...
                            + " $ "
                            + " ".join(
                                [str(x) for x in crucial[hk] if str(x).isprintable()]
                            )
                        )
                    else:
                        newln = (
            ...
            if len(keys) > 0:
                ...
                for ln in range(len(vals[keys[0]])):
                    target.write(
                        " ".join(
                            ["#LOG#"]
                            + [str(round(vals[k][ln], 5)) for k in keys]
                            + ["\n"]
                        )
                    )
    ...
    def readHeader(self, lines):
        ...
        for entry in lines:
            if "#" in entry:
                ...
                subentry = entry
            key, value = subentry.split("$")[0].strip(" "), subentry.split("$")[
                1
            ].strip(" ")
    ...
    def write_yaml(self, fname, full_output=False, compressed=False):
        ...
        try:
            data1["start_points"] = (
                temp[0].tostring(self.date_formatstring),
                temp[1].tostring(self.time_formatstring),
            )
        except:
        ...
        data1["fitting_params_absEnergy"] = [
            (str(k), float(self.fitting_params_absEnergy[k]))
            for k in self.fitting_params_absEnergy
        ]
        data1["fitting_params_channels"] = [
            (str(k), float(self.fitting_params_channels[k]))
            for k in self.fitting_params_channels
        ]
        data1["fitting_params_eV"] = [
            (str(k), float(self.fitting_params_eV[k])) for k in self.fitting_params_eV
        ]
    ...
    def read_yaml(self, fname):
        ...
        try:
            self.start_points = (
                QDate.fromString(temp[0], self.date_formatstring),
                QTime.fromString(temp[1], self.time.formatstring),
            )
        except:
    ...
    def write_hdf5(self, fname, full_output=False):
        ...
        mainentry = f.create_group("RIXSscan")
        header = mainentry.create_group("header")
        history = mainentry.create_group("history")
        ...
        logdata = mainentry.create_group("log")
        ...
        profs = [
            self.profile_channels[:, 1],
            self.profile_channels[:, 0],
            self.profile_absEnergy[:, 0],
            self.profile_eV[:, 0],
        ]
    ...
    def reconstruct_profiles(self):
        ...
        if not len(self.calibration_params) > 1:
            if len(self.calibration) > 1:
                ...
                print(
                    "not enough information to reconstruct absolute energy in ",
                    self.sourcename,
                )
                return None
        else:
        ...
        try:
            ...
            print(
                "not enough information to reconstruct the energy transfer axis in ",
                self.sourcename,
            )
        else:
        ...
        if "fits" in extension.lower():
            try:
                ...
                print(
                    "Thought that "
                    + fname
                    + " was a FITS file, but ReadFits did not work..."
                )
            ...
            try:
                ...
                print(
                    "Thought that "
                    + fname
                    + " was an old 2D ASC file, but ReadAsc did not work..."
                )
                data, header = ReadAndor(fname)
        else:
            data, header = ReadAndor(fname)
    ...
    def postprocess(self):
        ...
        shortnames = []
        headers, logs = [], []
        header, vallog = DataGroup(
            default_class=DataGroup, label="Header"
        ...
        for hk in header.keys():
            ...
            try:
                ...
                textheader += " ".join(
                    ["#", hk]
                    + [str(round(x, 4)) for x in [tmean, tmin, tmax, tstd]]
                    + ["\n"]
                )
    ...
    def makeSeparateProfiles(self, redfactor=1, cuts=None):
        for n, data in enumerate(self.data):
            ...
            self.individual_profiles_channels[n] = make_profile(
                data, reduction=redfactor, limits=cuts
            )
        self.addHistory(
            "makeSeparateProfiles, redfactor=" + str(redfactor) + ", cuts=" + str(cuts)
        )
    ...
    def makeEnergyCalibration(self, fitorder=2):
        ...
        pfit, pcov, infodict, errmsg, success = leastsq(
            fit_polynomial,
            np.zeros(order),
            args=(self.calibration[:, 0:2],),
            full_output=1,
        )
        pfit2, pcov2, infodict2, errmsg2, success2 = leastsq(
            fit_polynomial,
            np.zeros(order),
            args=(self.calibration[:, 2::-1],),
            full_output=1,
        )
    ...
    def eline(self):
        if len(self.reverse_calibration_params) > 0:
            self.nominal_elastic_line_position = polynomial(
                self.reverse_calibration_params, [self.energy]
            )
    ...
    def reconstructPeak(self):
        ...
        fitstring = " ".join(
            [
                "FWHM =",
                str(abs(round(fwhm, 3))),
                " +/- ",
                str(abs(round(fwhm_err, 3))),
                "\n",
                "centre =",
                str(round(centre, 3)),
                " +/- ",
                str(abs(round(centre_err, 3))),
                "\n",
                "area =",
                str(abs(round(area, 3))),
                " +/- ",
                str(abs(round(area_err, 3))),
            ]
        )  # , '\n',
        ...
        crit = np.where(
            np.logical_and(
                prof[:, 0] >= centre - 2 * fwhm, prof[:, 0] <= centre + 2 * fwhm
            )
        )
        oldx = prof[:, 0][crit]
        oldy = prof[:, 1][crit]
    ...
    def fitPeak(self, manual_limits=None, bkg_perc=50.0, redfac=1.0):
        guess = self.eline()
        bkg = self.profile_channels[
            np.where(
                self.profile_channels[:, 1]
                < np.percentile(self.profile_channels[:, 1], bkg_perc)
            )
        ]
        self.background_channels = bkg
        if manual_limits is not None:
            lim1, lim2 = manual_limits[0], manual_limits[1]
            fit, peak, chi2 = elastic_line(
                self.profile_channels, bkg, olimits=(lim1, lim2)
            )
        elif guess is None:
                ...
                for shift in np.arange(-20, 21, 10):
                    for ssize in [7, 15, 30]:
                        ...
                        if fit is not None:
                            ...
                            vallist.append(
                                (fwhm_quality + centre_quality) / abs(centre)
                            )
        ...
        if fit is not None:
            ...
            peak_area_error = (
                peak_area
                * ((fit[1][0] / fit[0][0]) ** 2 + (fit[1][1] / fit[0][1]) ** 2) ** 0.5
            )
            fitstring = " ".join(
                [
                    "FWHM =",
                    str(abs(round(fit[0][1], 3))),
                    " +/- ",
                    str(abs(round(fit[1][1], 3))),
                    "\n",
                    "centre =",
                    str(round(fit[0][2], 3)),
                    " +/- ",
                    str(abs(round(fit[1][2], 3))),
                    "\n",
                    "area =",
                    str(abs(round(peak_area, 3))),
                    " +/- ",
                    str(abs(round(peak_area_error, 3))),
                ]
            )  # , '\n',
            # '$\chi^{2}$=', str(round(chi2, 3))])
        else:
    ...
    def makePeakInEnergy(self):
        ...
        recalc = interp1d(
            self.profile_channels[:, 0], absE, fill_value=0.0, bounds_error=False
        )
        peak[:, 0] = polynomial(self.calibration_params, peak[:, 0])
        if len(self.background_channels) > 0:
            self.background_absEnergy = np.column_stack(
                [recalc(self.background_channels[:, 0]), self.background_channels[:, 1]]
            )
        ...
        self.fitting_textstring_absEnergy = " ".join(
            [
                "FWHM =",
                str(abs(round(newfwhm, 3))),
                " +/- ",
                str(abs(round(newfwhm_error, 3))),
                "\n",
                "centre =",
                str(round(newcentre, 3)),
                " +/- ",
                str(abs(round(newcentre_error, 3))),
                "\n",
                "area =",
                str(abs(round(newarea, 3))),
                " +/- ",
                str(abs(round(newarea_error, 3))),
            ]
        )  # , '\n',
        altpeak = peak.copy()
        ...
        self.fitting_textstring_eV = " ".join(
            [
                "FWHM =",
                str(abs(round(1000.0 * newfwhm, 3))),
                " +/- ",
                str(abs(round(1000.0 * newfwhm_error, 3))),
                " meV\n",
                "centre =",
                str(round(0.0, 3)),
                " +/- ",
    ...
    def makeEnergyProfile(self, fitorder=2):
        ...
        self.addHistory(
            "Converted to energy, with calibration parameters: "
            + str(self.calibration_params)
        )
    ...
    def fitCurvature(self, limits=None, segsize=16, bkg_perc=65):
        ...
        if self.merged_data is not None:
            curve = curvature_profile(
                self.merged_data, blocksize=segsize, percentile=bkg_perc, olimits=limits
            )
            pfit, pcov, infodict, errmsg, success = leastsq(
                fit_polynomial,
                [curve[:, 1].mean(), 0.0, 0.0],
                args=(curve,),
                full_output=1,
            )
    ...
    def fakeData(self, cuts, bpp):
        ...
        try:
            ...
            data = (
                rand_mt.standard_normal(size=(width * height)) * 0.1 * bpp + 0.2 * bpp
            )
    ...
    def __add__(self, other):
        ...
        temp.individual_profiles_channels = (
            self.individual_profiles_channels + other.individual_profiles_channels
        )
        temp.individual_profiles_absEnergy = (
            self.individual_profiles_absEnergy + other.individual_profiles_absEnergy
        )
        temp.individual_profiles_eV = (
            self.individual_profiles_eV + other.individual_profiles_eV
        )
        profs1 = [
            self.profile_channels.copy(),
            self.profile_eV.copy(),
            self.profile_absEnergy.copy(),
        ]
        profs2 = [
            other.profile_channels.copy(),
            other.profile_eV.copy(),
            other.profile_absEnergy.copy(),
        ]
        ...
        for nn in range(3):
            ...
            if len(tp1) == 0:
                ...
                if len(tp2) == 0:
                    ...
                    the_object = MergeManyCurves(
                        [tp1, tp2], target, mthreads=self.maxthreads
                    )
                    the_object.runit()
