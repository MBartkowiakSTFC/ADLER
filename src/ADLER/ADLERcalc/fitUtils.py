...
def fit_polynomial(args, array):
    ...
    if array.shape[-1] > 2:
        return (newfit - array[:, 1]) / (
            array[:, 2] + np.abs(0.00001 * array[:, 1].mean())
        )
    else:
        return newfit - array[:, 1]
...
if has_voigt:
    ...
    def my_voigt(xarr, A_in, xc_in, wG_in, wL_in):
        ...
        temp_y = (
            A
            * np.real(wofz((temp_x + 1j * wL) / (wG * np.sqrt(2))))
            / (wG * np.sqrt(2 * np.pi))
        )
        temp_func = interp1d(temp_x + xc, temp_y, bounds_error=False, fill_value=0.0)
        return temp_func(xarr)
    ...
    def fast_voigt(xarr, A_in, xc_in, wG_in, wL_in):
        ...
        temp_x = xarr - xc
        # temp_y = voigt_profile(temp_x, wG, wL) *A
        temp_y = np.real(wofz((temp_x + 1j * wL) / (wG * np.sqrt(2)))) / (
            wG * np.sqrt(2 * np.pi)
        )
        temp_y *= A / temp_y.max()
        return temp_y
    ...
    def asym_voigt(xarr, A_in, xc_in, wG_in, wL_in, R):
        ...
        if len(left[0]) > 0:
            templeft = np.real(
                wofz(
                    (temp_x[left] + 1j * widths_left[1]) / (widths_left[0] * np.sqrt(2))
                )
            ) / (widths_left[0] * np.sqrt(2 * np.pi))
            temp_y[left] += templeft
        if len(right[0]) > 0:
            tempright = np.real(
                wofz(
                    (temp_x[right] + 1j * widths_right[1])
                    / (widths_right[0] * np.sqrt(2))
                )
            ) / (widths_right[0] * np.sqrt(2 * np.pi))
            tempright *= templeft.max() / tempright.max()
            temp_y[right] += tempright
        temp_y *= abs(A) / temp_y.max()
        return temp_y
...
def single_flex_profile_cf(
    ...
    profile = single_flex_profile(
        params, xarr, yarr, errarr, npeaks, bkg_order, include_edge, penalty
    )
...
def single_flex_profile(
    ...
    for n in range(npeaks):
        profile += asym_voigt(
            ...
            abs(params[n]),
            params[n + npeaks],
            abs(params[n + 2 * npeaks]),
            abs(params[n + 3 * npeaks]),
            params[-1],
        )
...
def single_background(params, xarr, bkg_order=0, include_edge=False):
    ...
    if include_edge:
        new_y += params[bkg_order] * np.arctan(
            (xarr - params[bkg_order + 1]) / params[bkg_order + 2]
        )
    return new_y
...
def fit_background(
    params, xarr, yarr, errarr=None, penalty=10.0, bkg_order=0, include_edge=False
):
    new_y = np.zeros(xarr.shape)
...
def global_fitting_optimiser(
    data,
    maxpeaks=10,
    bkg_order=0,
    include_edge=False,
    one_gauss_fwhm=False,
    one_lorentz_fwhm=False,
    ...
    if ncol < 2:
        ...
        print(
            "Fitting the profile using column 1 as x, column 2 as y, and column 3 as y-error."
        )
        errors = True
    else:
    ...
    for peakn in np.arange(maxpeaks):
        ...
        for bo in np.arange(bkg_order + 1):
            if bo == 0:
                ...
                bounds += [
                    (-0.3 * abs(Y_span) ** (1 / bo), 0.3 * abs(Y_span) ** (1 / bo))
                ]
        if include_edge:
            bounds += [(-Y_span, Y_span), (xarr.min(), xarr.max()), (0.0, X_span / 2.0)]
        print(bounds)
        results = shgo(
            single_flex_profile_cf,
            bounds,
            args=(
                xarr,
                yarr,
        ...
        costfunc = costfunction_no_overshoot(
            test, yarr, errarr=None, penalty=overshoot_penalty
        )
...
def iterative_fitting_optimiser(
    data,
    maxpeaks=10,
    bkg_order=0,
    include_edge=False,
    ...
    for peakn in np.arange(maxpeaks):
        ...
        for p in np.arange(peakn):
            ...
            for tr in testranges:
                ...
                result = minimize(
                    fit_voigt,
                    [pheight, pcentre, last_gwidth, last_lwidth],
                    args=(temp_xarr, temp_y, temp_err, overshoot_penalty),
                )
                pfit = result["x"]
                ...
                fits.append(pfit)
                intensities.append(
                    fast_voigt(reduced_x, pfit[0], pfit[1], pfit[2], pfit[3]).sum()
                    / (tr[1] - tr[0])
                )
        ...
        init_params = np.concatenate(
            [
                peak_params[0 : 4 * peakn : 4],
                peak_params[1 : 4 * peakn : 4],
                peak_params[2 : 4 * peakn : 4],
                peak_params[3 : 4 * peakn : 4],
            ]
        )
        #        for p in np.arange(peakn):
