...
loading_variables = [
    {
        "Name": "Detector cutoff",
        "Unit": "pixel",
        "Value": np.array([0, 2048]),
        "Key": "cuts",
    ...
    {
        "Name": "Cosmic ray correction factor",
        "Unit": "StdDev",
        "Value": 3,
        "Key": "cray",
        "MinValue": -10.0,
...
energy_variables = [
    {
        "Name": "Energy calibration pair 1",
        "Unit": "pixel, eV",
    ...
    {
        "Name": "Energy calibration pair 2",
        "Unit": "pixel, eV",
        "Value": np.array([-1.0, -1.0]),
        "Key": "eVpair2",
        "MinValue": -1e5 * np.ones(2),
...
curvature_vars = [
    [
        "Input File(s)",
        ["Curvature", "IFname"],
        "Andor SIF file(s) to be processed.",
        "",
    ],
    [
        "Lower vertical cutoff [channel]",
        ["Curvature", "LCut"],
        "First pixel column to be included in the 1D profile",
        "0",
    ],
    [
        "Upper vertical cutoff [channel]",
        ["Curvature", "HCut"],
        "Last pixel column to be included in the 1D profile",
        "2048",
    ],
    [
        "Elastic line position [channel]",
        ["Curvature", "ELine"],
        "Initial guess of the elastic line position for fitting.\n"
    ...
    [
        ...
        ["Curvature", "BkgPP"],
        "This value will be subtracted from each pixel. Will be guessed if left empty",
        "900",
    ],
    [
        "Background limit (percentile)",
        ["Curvature", "BkgPercentile"],
        "All the points with values below this percentile will be considered background.",
        "75",
    ],
    [
        "Cosmic ray correction factor",
        ["Curvature", "CRay"],
        "For each pixel row, only values within THIS_NUMBER * StdDev from the mean value will be accepted.",
        "5",
    ],
    [
        "Pixels per segment for curvature profile",
        ["Curvature", "Blocksize"],
        "This is the number of vertical channels that will be summed up to produce one point in the curvature plot.",
        "16",
    ],
    [
        "Polynomial coefficients",
        ["Curvature", "Poly"],
        "If you already know the curvature parameters, you can put them here directly",
        "None",
    ],
    [
        "Cut FFT regions",
        ["Curvature", "FFTCuts"],
        "Regions to be removed from data in the reciprocal space",
        "None",
    ],
]
...
server_vars = [
    [
        "Working Directory",
        ["Server", "WDir"],
        "Working directory where the server will wait for new files to appear.",
        "",
    ],
    [
        "First number",
        ["Server", "NLow"],
        "If specified, processing will start with a file name containing this number",
        "",
    ],
    [
        "Last number",
        ["Server", "NHigh"],
        "If specified, processing will finish on a file name containing this number",
        "",
    ],
]
...
class InterfaceTabbed:
    def __init__(self, master, size, param, appinstance=None, pathdict={}):
        ...
        self.logger(
            "<b>You are using ADLER version " + str(ADLER_VERSION_STRING) + "</b>"
        )
        self.logger(
            "When in doubt about a part of the gui, refer to the <b>hovertext</b>."
        )
        self.logger("You could also try reading the <b>manual</b>.")
        self.logger(
            "The configuration file .ADLconfig.txt is stored in your home directory."
        )
        ...
        viewertab = LogviewerTab(
            self.tabbar,
            self.MakeCanvas(self.tabbar),
            self.log,
            startpath=paths["variableviewer"],
        )
        beamlinetab = BeamlineTab(
            self.tabbar,
            self.MakeCanvas(self.tabbar),
            self.log,
            startpath=paths["beamline"],
        )
        self.STab = SingleTab(
            self.tabbar,
            self.MakeCanvas(self.tabbar),
            self.log,
            mthreads=self.max_threads,
            startpath=paths["data"],
            logplotter=viewertab,
            app=appinstance,
        )
        self.CTab = CorrectionsTab(
            self.tabbar,
            self.MakeCanvas(self.tabbar),
            self.log,
            mthreads=self.max_threads,
            startpath=paths["corrections"],
            app=appinstance,
        )
        self.PTab = PostprocessingTab(
            self.tabbar,
            self.MakeCanvas(self.tabbar),
            self.log,
            mthreads=self.max_threads,
            startpath=paths["postprocessing"],
            app=appinstance,
        )
        self.XTab = XASplottingTab(
            self.tabbar,
            self.MakeCanvas(self.tabbar),
            self.log,
            mthreads=self.max_threads,
            startpath=paths["xasplotting"],
            app=appinstance,
        )
        self.FitTab = FittingTab(
            self.tabbar,
            self.MakeCanvas(self.tabbar),
            self.log,
            mthreads=self.max_threads,
            startpath=paths["fitting"],
            app=appinstance,
        )
    ...
    def MakeFileTab(self):
        ...
        button_list = [
            [
                "Pick Directory",
                self.PickFileDir,
                "Select a directory containing PEAXIS data files.",
                None,
                "Files",
            ],  # 0,
            [
                "Load Files",
                self.PassSelectedFiles,
                "Load the selected files into the RIXS Data Reduction tab.",
                None,
                "Files",
            ],  # 0,
        ]
        ...
        self.filemodel = PeaxisDataModel(
            tab1,
            [
                "name",
                "Photon energy",
...
def startGUI(some_args, pdict=None):
    ...
    root.setWindowTitle("ADLER 4.1: Augmented Data Loading, Evaluation and Reduction.")
    ...
    interface = InterfaceTabbed(
        root, (800, 700), params, appinstance=app, pathdict=pdict
    )
