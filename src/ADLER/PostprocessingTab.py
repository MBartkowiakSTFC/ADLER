...
from ADLER.ADLERcalc.SimpleCore import SimpleCore
...
loading_variables = [
    {
        "Name": "Spectrum cutoff",
        "Unit": "pixel",
        "Value": np.array([0, 2048]),
        "Key": "cuts",
        "MinValue": np.array([-1e5, -1e5]),
        "MaxValue": np.array([1e5, 1e5]),
        "Length": 2,
        "Type": "int",
            ...
            for nc in range(0, 8):
                ...
                if nc == 1:
                    ...
                    try:
                        vals = [
                            float(tok.strip("()[]'"))
                            for tok in self.item(nr, nc).text().split(",")
                        ]
                    except:
                        continue
                    self.Xmin[nr] = vals[0]
                    ...
                    self.temperature[nr] = float(
                        self.item(nr, nc).text().strip("()[]'")
                    )
                elif nc == 5:
                    ...
                    self.useit[nr] = (
                        self.item(nr, nc).checkState() == Qt.CheckState.Checked
                    )
...
class PostprocessingTab(AdlerTab):
    ...
    def finish_loading(self, parlist):
        ...
        for n in range(len(snames)):
            ...
            self.profile_list.add_row(
                [
                    snames[n],
                    envals[n],
                    [xmin, xmax],
                    units[n],
                    pardicts[n]["temperature"],
                    pardicts[n]["2theta"],
                    pardicts[n]["Q"],
                ]
            )
    ...
    def add_mergedcurve(self):
        ...
        self.profile_list.add_row(
            [
                "Merged data",
                self.core.merged_energy,
                [xmin, xmax],
                self.core.merged_units,
                self.core.merged_temperature,
                self.core.merged_2theta,
                self.core.merged_q,
            ]
        )
        self.profile_list.update_values()
    ...
    def add_filteredcurves(self):
        for num, curve in enumerate(self.core.filter_curves):
            ...
            self.profile_list.add_row(
                [
                    self.core.filter_labels[num],
                    self.core.filter_energies[num],
                    [xmin, xmax],
                    self.core.filter_units[num],
                    self.core.filter_temperatures[num],
                    self.core.filter_2thetas[num],
                    self.core.filter_qs[num],
                ]
            )
        self.profile_list.update_values()
        # self.profile_list.redraw_table()
